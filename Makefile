...
CANFESTIVAL_LIB = $(CANFESTIVAL)/bin/libcanfestival.a
...
CFLAGS += -g -W -Wall -Wstrict-prototypes -Werror -O2 $(INCLUDES)
...
distclean:
	$(MAKE) -C $(CANFESTIVAL) distclean

.PHONY: all clean distclean objedit


dictionary.c dictionary.h: $(TARGET:=.od)
	python $(CANFESTIVAL)/objdictgen/objdictgen.py $< $@

$(CANFESTIVAL_LIB) $(CANFESTIVAL_TARGET_LIB): $(CANFESTIVAL)/Makefile
	$(MAKE) -C $(CANFESTIVAL) directories canfestival

$(CANFESTIVAL)/Makefile:
